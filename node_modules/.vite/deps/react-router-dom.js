...
function createRouter(init) {
  ...
  async function startNavigation(historyAction, location2, opts) {
    ...
    if (opts && opts.pendingError) {
      ...
      let actionResult = await handleAction(request, location2, opts.submission, matches, scopedContext, fogOfWar.active, opts && opts.initialHydration === true, { replace: opts.replace, flushSync: flushSync3 });
    ...
    let {
      ...
      loaderData,
      errors
    } = await handleLoaders(request, location2, matches, scopedContext, fogOfWar.active, loadingNavigation, opts && opts.submission, opts && opts.fetcherSubmission, opts && opts.replace, opts && opts.initialHydration === true, flushSync3, pendingActionResult);
    if (shortCircuited) {
      return;
    }
    pendingNavigationController = null;
  ...
  async function fetch2(key, routeId, href2, opts) {
    ...
    if (!matches) {
      setFetcherError(key, routeId, getInternalRouterError(404, { pathname: normalizedPath }), { flushSync: flushSync3 });
      return;
    }
    ...
    if (submission && isMutationMethod(submission.formMethod)) {
      await handleFetcherAction(key, routeId, path, matches, scopedContext, fogOfWar.active, flushSync3, preventScrollReset, submission);
      return;
    }
    fetchLoadMatches.set(key, { routeId, path });
    await handleFetcherLoader(key, routeId, path, matches, scopedContext, fogOfWar.active, flushSync3, preventScrollReset, submission);
  }
  async function handleFetcherAction(key, routeId, path, requestMatches, scopedContext, isFogOfWar, flushSync3, preventScrollReset, submission) {
    ...
    if (isFogOfWar) {
      ...
      if (discoverResult.type === "aborted") {
        ...
        setFetcherError(key, routeId, getInternalRouterError(404, { pathname: path }), { flushSync: flushSync3 });
        return;
      } else {
  ...
  async function handleFetcherLoader(key, routeId, path, matches, scopedContext, isFogOfWar, flushSync3, preventScrollReset, submission) {
    let existingFetcher = state.fetchers.get(key);
    updateFetcherState(key, getLoadingFetcher(submission, existingFetcher ? existingFetcher.data : void 0), { flushSync: flushSync3 });
    let abortController = new AbortController();
  ...
  if (init.unstable_instrumentations) {
    router2 = instrumentClientSideRouter(router2, init.unstable_instrumentations.map((i) => i.router).filter(Boolean));
  }
  return router2;
}
      ...
      try {
        ...
        let response = await runServerMiddlewarePipeline({
          ...
          let res = await generateMiddlewareResponse(async (revalidationRequest, opts2 = {}) => {
            var _a;
            let result2 = await queryImpl(revalidationRequest, location2, matches, requestContext, dataStrategy || null, skipLoaderErrorBubbling === true, null, "filterMatchesToLoad" in opts2 ? (_a = opts2.filterMatchesToLoad) != null ? _a : null : filterMatchesToLoad != null ? filterMatchesToLoad : null, skipRevalidation === true);
...
function dataWithResponseInitToErrorResponse(data2) {
  var _a, _b, _c, _d;
  return new ErrorResponseImpl((_b = (_a = data2.init) == null ? void 0 : _a.status) != null ? _b : 500, (_d = (_c = data2.init) == null ? void 0 : _c.statusText) != null ? _d : "Internal Server Error", data2.data);
}
...
function useOutlet(context) {
  ...
  return React2.useMemo(() => outlet && React2.createElement(OutletContext.Provider, { value: context }, outlet), [outlet, context]);
}
function useParams() {
  let { matches } = React2.useContext(RouteContext);
...
function RouterProvider({
  ...
  let setState = React3.useCallback((newState, { deletedFetchers, flushSync: flushSync3, viewTransitionOpts }) => {
    ...
    warnOnce(flushSync3 === false || reactDomFlushSyncImpl != null, 'You provided the `flushSync` option to a router update, but you are not using the `<RouterProvider>` from `react-router/dom` so `ReactDOM.flushSync()` is unavailable.  Please update your app to `import { RouterProvider } from "react-router/dom"` and ensure you have `react-dom` installed as a dependency to use the `flushSync` option.');
...
function Await({
  ...
  return React3.createElement(AwaitErrorBoundary, {
    resolve,
    errorElement,
        ...
        while (Object.keys(encoder3.deferred).length > 0) {
          for (const [deferredId, deferred] of Object.entries(encoder3.deferred)) {
            if (seenPromises.has(deferred))
              continue;
            seenPromises.add(encoder3.deferred[Number(deferredId)] = Promise.race([
              racePromise,
              deferred
            ]).then((resolved) => {
...
function getSingleFetchDataStrategyImpl(getRouter, getRouteInfo, fetchAndDecode, ssr, basename, shouldAllowOptOut = () => true) {
  return async (args) => {
    ...
    return singleFetchLoaderNavigationStrategy(args, router2, getRouteInfo, fetchAndDecode, ssr, basename, shouldAllowOptOut);
  };
}
...
function RemixRootDefaultHydrateFallback() {
  return React5.createElement(BoundaryShell, { title: "Loading...", renderScripts: true }, ENABLE_DEV_WARNINGS ? React5.createElement("script", {
    dangerouslySetInnerHTML: {
      __html: `
              console.log(
...
function Links({ nonce }) {
  ...
  return React8.createElement(React8.Fragment, null, typeof criticalCss === "string" ? React8.createElement("style", {
    ...{ [CRITICAL_CSS_DATA_ATTRIBUTE]: "" },
    dangerouslySetInnerHTML: { __html: criticalCss }
  }) : null, typeof criticalCss === "object" ? React8.createElement("link", {
    ...{ [CRITICAL_CSS_DATA_ATTRIBUTE]: "" },
    rel: "stylesheet",
    href: criticalCss.href,
    nonce
  }) : null, keyedLinks.map(({ key, link }) => isPageLinkDescriptor(link) ? React8.createElement(PrefetchPageLinks, { key, nonce, ...link }) : React8.createElement("link", { key, nonce, ...link })));
}
...
import(${JSON.stringify(manifest.entry.module)});`;
    return React8.createElement(React8.Fragment, null, React8.createElement("script", {
      ...scriptProps,
  ...
  return isHydrated || isRSCRouterContext ? null : React8.createElement(React8.Fragment, null, typeof manifest.sri === "object" ? React8.createElement("script", {
    "rr-importmap": "",
    type: "importmap",
    suppressHydrationWarning: true,
    dangerouslySetInnerHTML: {
  ...
  }) : null, !enableFogOfWar ? React8.createElement("link", {
    rel: "modulepreload",
    href: manifest.url,
    ...
    suppressHydrationWarning: true
  }), preloads.map((path) => React8.createElement("link", {
    key: path,
    rel: "modulepreload",
    href: path,
    crossOrigin: scriptProps.crossOrigin,
...
function RemixRootDefaultErrorBoundary({
  ...
  return React9.createElement(BoundaryShell, {
    title: "Application Error!",
    isOutsideRemixApp
  }, React9.createElement("h1", { style: { fontSize: "24px" } }, "Application Error"), React9.createElement("pre", {
    style: {
      padding: "2rem",
...
function BoundaryShell({
  ...
  return React9.createElement("html", { lang: "en" }, React9.createElement("head", null, React9.createElement("meta", { charSet: "utf-8" }), React9.createElement("meta", {
    name: "viewport",
...
function useScrollRestoration({
  ...
  if (typeof document !== "undefined") {
    ...
    React10.useLayoutEffect(() => {
      let disableScrollRestoration = router2 == null ? void 0 : router2.enableScrollRestoration(savedScrollPositions, () => window.scrollY, getKey ? (location22, matches2) => getScrollRestorationKey(location22, matches2, basename, getKey) : void 0);
...
function createRoutesStub(routes, _context) {
  return function RoutesTestStub({
    ...
    if (routerRef.current == null) {
      ...
      let patched = processRoutes(convertRoutesToDataRoutes(routes, (r) => r), _context !== void 0 ? _context : (future == null ? void 0 : future.v8_middleware) ? new RouterContextProvider() : {}, frameworkContextRef.current.manifest, frameworkContextRef.current.routeModules);
...
function ErrorWrapper({
  ...
  return import_react.default.createElement("html", { lang: "en" }, import_react.default.createElement("head", null, import_react.default.createElement("meta", { charSet: "utf-8" }), import_react.default.createElement("meta", {
    name: "viewport",
    content: "width=device-width,initial-scale=1,viewport-fit=cover"
...
function RSCDefaultRootErrorBoundaryImpl({
  ...
  if (isRouteErrorResponse(error)) {
    return import_react.default.createElement(ErrorWrapper, {
      renderAppShell,
      title: "Unhandled Thrown Response!"
    }, import_react.default.createElement("h1", { style: { fontSize: "24px" } }, error.status, " ", error.statusText), ENABLE_DEV_WARNINGS ? heyDeveloper : null);
  }
  let errorInstance;
  if (error instanceof Error) {
    errorInstance = error;
  ...
  return import_react.default.createElement(ErrorWrapper, { renderAppShell, title: "Application Error!" }, import_react.default.createElement("h1", { style: { fontSize: "24px" } }, "Application Error"), import_react.default.createElement("pre", {
    style: {
      padding: "2rem",
      background: "hsla(10, 50%, 50%, 0.1)",
      color: "red",
...
function RSCStaticRouter({ getPayload }) {
  ...
  const router2 = createStaticRouter(payload.matches.reduceRight((previous, match) => {
    const route = {
      id: match.id,
      action: match.hasAction || !!match.clientAction,
  ...
  return React42.createElement(RSCRouterContext.Provider, { value: true }, React42.createElement(RSCRouterGlobalErrorBoundary, { location: payload.location }, React42.createElement(FrameworkContext.Provider, { value: frameworkContext }, React42.createElement(StaticRouterProvider, {
    context,
    router: router2,
    hydrate: false,
    nonce: payload.nonce
...
function HydratedRouter(props) {
  ...
  let [criticalCss, setCriticalCss] = React23.useState(true ? ssrInfo == null ? void 0 : ssrInfo.context.criticalCss : void 0);
  React23.useEffect(() => {
    if (true) {
  ...
  return React23.createElement(React23.Fragment, null, React23.createElement(FrameworkContext.Provider, {
    value: {
  ...
  }, React23.createElement(RemixErrorBoundary, { location: location2 }, React23.createElement(RouterProvider2, {
    router,
